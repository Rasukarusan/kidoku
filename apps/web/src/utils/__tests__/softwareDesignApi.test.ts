...
describe('softwareDesignApi', () => {
  describe('isSoftwareDesignISBN', () => {
    ...
    it('タイトルでSoftware Designを判定する', () => {
      expect(isSoftwareDesignISBN('978-4-123-45678-9', 'Software Design 2025年7月号')).toBe(true)
      expect(isSoftwareDesignISBN('978-4-123-45678-9', 'ソフトウェアデザイン')).toBe(true)
      expect(isSoftwareDesignISBN('978-4-123-45678-9', 'Web+DB PRESS')).toBe(false)
    })
  })
  ...
  describe('generateSoftwareDesignImageUrl', () => {
    it('正しい画像URLを生成する', () => {
      ...
      expect(url).toBe('https://gihyo.jp/assets/images/cover/2025/thumb/TH800_642507.jpg')
    })

    it('1桁の月でも正しくパディングする', () => {
      const url = generateSoftwareDesignImageUrl(2025, 1)
      expect(url).toBe('https://gihyo.jp/assets/images/cover/2025/thumb/TH800_642501.jpg')
    })
  })
