...
export const LoadingAiSummary: React.FC<Props> = ({ jsonKey, text }) => {
...
export const Loading: React.FC = () => {
  ...
  useEffect(() => {
    const speed = 100
    const delay = [7, 20, 32, 45] // 単位は秒
    setTimeout(() => {
      const reading_trend_analysis =
        '「読書傾向」は、ユーザーが好むジャンル、著者、テーマを分析し、その傾向を可視化します。これによって今、自分が何を好きなのか、どんなジャンルに対しては前向きなのかなどがわかります。'
      for (let i = 0; i <= reading_trend_analysis.length; i++) {
        setTimeout(() => {
          setJson((prevJson) => ({
            ...prevJson,
            reading_trend_analysis: reading_trend_analysis.slice(0, i),
          }))
        }, i * speed)
      }
    }, delay[0] * 1000)
    ...
    setTimeout(() => {
      ...
      for (let i = 0; i <= sentiment_analysis.length; i++) {
        setTimeout(() => {
          setJson((prevJson) => ({
            ...prevJson,
            sentiment_analysis: sentiment_analysis.slice(0, i),
          }))
        }, i * speed)
      }
    }, delay[1] * 1000)
    ...
    setTimeout(() => {
      const what_if_scenario =
        '「もしもシナリオ」は、ユーザーが異なるジャンルや著者を選んだ場合にどのような読書体験になるかを想像させる、仮想シナリオを提示します。'
      for (let i = 0; i <= what_if_scenario.length; i++) {
        setTimeout(() => {
          setJson((prevJson) => ({
            ...prevJson,
            what_if_scenario: what_if_scenario.slice(0, i),
          }))
        }, i * speed)
      }
    }, delay[2] * 1000)
    setTimeout(() => {
      const overall_feedback = '「まとめ」は、まとめです。'
      for (let i = 0; i <= overall_feedback.length; i++) {
        setTimeout(() => {
          setJson((prevJson) => ({
            ...prevJson,
            overall_feedback: overall_feedback.slice(0, i),
          }))
        }, i * speed)
      }
    }, delay[3] * 1000)
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          {Object.keys(json).map((key, i) => {
            ...
            return (
              ...
              >
                <LoadingAiSummary key={key} jsonKey={key} text={json[key]} />
              </motion.div>
            )
          })}
        </div>
