...
function getLayer(srcDir, filePath) {
  ...
  if (relative.startsWith('presentation/modules/'))
    return 'presentation-modules';
 ...
 */
function createDddLayerBoundariesRule(srcDir) {
  return {
    meta: {
      type: 'problem',
      docs: {
    ...
    create(context) {
      return {
        ImportDeclaration(node) {
          ...
          const fromLayer = getLayer(srcDir, currentFile);
          ...
          const toLayer = getLayer(srcDir, importedPath);
          if (!toLayer) return;
 ...
 */
export function createApiConfig({ tsconfigRootDir, srcDir }) {
  const dddBoundariesPlugin = {
    rules: {
      'layer-boundaries': createDddLayerBoundariesRule(srcDir),
    },
  };
